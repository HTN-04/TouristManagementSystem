    ...
    public List<ChuDichVu> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ChuDichVu cdv = new ChuDichVu(ten, tenDangNhap, matKhau, email, sdt,vaiTro);
                chuDichVus.add(cdv);
            }
...
//        System.out.println(role);
     ...
     //the same with selctByTenDangNhap
     public ChuDichVu getThongTinTaiKhoan(String tenDangNhap){
         ChuDichVu chuDichVu = null;
         
         Connection conn = DatabaseUtils.getConnection();
         String sql = "select * from CHU_DICH_VU where TenDangNhap = ?";
        try {
            PreparedStatement smt = conn.prepareStatement(sql);
            smt.setString(1, tenDangNhap);
            ResultSet rs =smt.executeQuery();
            while(rs.next()){
                String tenCdv = rs.getString("TenCDV");
                String tdn = rs.getString("TenDangNhap");
                String matKhau = rs.getString("MatKhau");
                String email = rs.getString("Email");
                String sdt = rs.getString("SoDienThoai");
                String vaiTro = rs.getString("VaiTro");
                chuDichVu = new ChuDichVu(tenCdv, tenDangNhap, matKhau, email, sdt, vaiTro);
            }
        } catch (SQLException ex) {
            Logger.getLogger(ChuDichVuDAO.class.getName()).log(Level.SEVERE, null, ex);
        } finally{
